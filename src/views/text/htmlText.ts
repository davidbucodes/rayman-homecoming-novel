...
/**
 * Modified copy of @pixi/text-html
 ...
 * @see {@link https://pixijs.download/dev/docs/PIXI.PIXI.TextStyle.html PIXI.PIXI.TextStyle}
 */
export class HTMLText extends PIXI.Sprite {
	...
	private _style: PIXI.TextStyle | null = null;
	 ...
	 * @param {PIXI.PIXI.TextStyle} [style] - Style PIXI.settings, not all PIXI.TextStyle options are supported.
	...
	constructor(
		...
		const texture = PIXI.Texture.from(canvas, { scaleMode: PIXI.settings.SCALE_MODE });
		...
		this._resolution = PIXI.settings.RESOLUTION;
		this.text = text;
		this.style = style;
	}
	...
	updateText(respectDirty = true): void {
		...
		if (style.strokeThickness) {
			...
			if (typeof stroke === "number") {
				stroke = PIXI.utils.hex2string(stroke);
			}
		...
		if (style.dropShadow) {
			...
			if (color.charAt(0) === "#" && dropShadowAlpha < 1) {
				const [r, g, b] = PIXI.utils.hex2rgb(parseInt(color.replace("#", ""), 16));

				color = `rgba(${(r * 255) | 0}, ${(g * 255) | 0}, ${
					(b * 255) | 0
				}, ${dropShadowAlpha})`;
			}
		...
		dom.setAttribute("dir", style.align === "right" ? "rtl" : "ltr");
	...
	updateTexture() {
		...
		if (style.trim) {
			const { width, height, data } = PIXI.utils.trimCanvas(canvas);
	 ...
	 *
	 * @param {PIXI.PIXI.Renderer} renderer - The renderer
	 * @private
	 */
	 ...
	 */
	_renderCanvas(renderer: PIXI.Renderer) {
		if (this._autoResolution && this._resolution !== renderer.resolution) {
			this._resolution = renderer.resolution;
			this.dirty = true;
		}
	 ...
	 *
	 * @param {PIXI.PIXI.Rectangle} rect - Input rectangle.
	 * @return {PIXI.PIXI.Rectangle} Local bounds
	 */
	 ...
	 * @member {PIXI.PIXI.TextStyle}
	 */
	get style(): PIXI.TextStyle {
		return this._style as PIXI.TextStyle;
	}

	set style(
		style: PIXI.TextStyle | Partial<PIXI.ITextStyle>, // eslint-disable-line require-jsdoc
	) {
		style = style || {};
